...
{
	float lengthV = length(aVec);
	return(Vec3f{ aVec.x / lengthV, aVec.y / lengthV, aVec.z / lengthV });

}


inline
Vec3f cross(Vec3f aVec, Vec3f aVec2) noexcept
{
	float cx = aVec.y * aVec2.z - aVec.z * aVec2.y;
	float cy = aVec.z * aVec2.x - aVec.x * aVec2.z;
	float cz = aVec.x * aVec2.y - aVec.y * aVec2.x;
